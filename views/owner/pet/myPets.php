	...
	if ($num > 0)
	{
		echo "<table class='table table-striped'>";
	  	...
	  	while ($row = pg_fetch_row($result)) {
			...
			echo "<tr onclick=\"window.location='Dashboard.php?p=owner/pet/modify_pet&object_id=" . urlencode($pet->getPetID()) . "'\" style='cursor: pointer;'>";
      ...
      if ($totalPages > 1)
      {
      	echo "<ul class='pagination'>";
        	if ($currentPage > 1) 
        	{ // Previous page link
         	echo "<li class='page-item'><a class='page-link' href='?p=owner/pet/myPets&page=" . ($currentPage - 1) . "'>Previous</a></li>";
        	}

        	for ($i = 1; $i <= $totalPages; $i++) { // Page number links
         	$activeClass = ($currentPage == $i) ? "active" : "";
         	echo "<li class='page-item " . $activeClass . "'><a class='page-link' href='?p=owner/pet/myPets&page=" . $i . "'>" . $i . "</a></li>";
        	}
	
     		if ($currentPage < $totalPages) { // Next page link
        		echo "<li class='page-item'><a class='page-link' href='?p=owner/pet/myPets&page=" . ($currentPage + 1) . "'>Next</a></li>";
     		}
     		echo "</ul>";
     	}
	} 
